...
pio.orca.config.executable = directory + 'plotly-orca-1.2.1-1/orca_app/orca.exe'
...
#    print('here')
#    df['spend'] = df['spend'].fillna(-1)
#    if (df['spend'].sum() <= 0) & ({'Rate', 'AnnualVolume'}.issubset(df.columns)):
#        df['Rate'] = pd.to_numeric(df['Rate'], errors='coerce')
#        df['AnnualVolume'] = pd.to_numeric(df['AnnualVolume'], errors='coerce')
#        df['spend'] = df['rate'] * df['AnnualVolume']
#    else:
#        pass
#elif {'Rate', 'AnnualVolume'}.issubset(df.columns):
#    df['Rate'] = pd.to_numeric(df['Rate'], errors='coerce')
#    df['AnnualVolume'] = pd.to_numeric(df['AnnualVolume'], errors='coerce')
#    df['spend'] = df['Rate'] * df['AnnualVolume']
#else:
...
def htmlToimg(file): 
    driver = webdriver.Chrome(directory+image_dir+'chromedriver.exe')
